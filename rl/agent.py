"""
Orbit Wars PPO Agent — Pointer Network for Target Selection.

Key innovation: Policy outputs target_planet_idx (pointer network) instead of
raw angle. The angle is computed from the selected target planet's position.

Architecture:
- Encoder: planet/fleet/global feature encoders (can warm-start from BC)
- Policy head per owned planet: (send_prob, target_logits, ship_ratio)
- Value head: V(s) for PPO baseline
"""
import math
import torch
...
class PPOAgent(nn.Module):
    """
    PPO agent for Orbit Wars with pointer network target selection.
    
    For each owned planet, outputs:
    - send_prob: probability of sending a fleet
    - target_logits: distribution over all planets (pointer network)
    - ship_ratio: fraction of ships to send (0-1)
    - value: V(s) scalar for baseline
    """
    def __init__(self, embed_dim=64, max_planets=40, max_fleets=100, num_players=2):
        ...
        # ─── Encoders (can be loaded from BC checkpoint) ───
        # Planet features: [id, owner, x, y, radius, ships, production] = 7 dims
        self.planet_enc = nn.Sequential(
            nn.Linear(7, embed_dim), nn.ReLU(),
            nn.Linear(embed_dim, embed_dim), nn.ReLU(),
        )
        
        # Fleet features: [id, owner, x, y, angle, from_planet_id, ships] = 7 dims
        self.fleet_enc = nn.Sequential(
            nn.Linear(7, embed_dim), nn.ReLU(),
            nn.Linear(embed_dim, embed_dim), nn.ReLU(),
        )
        
        # Global context: planet_summary(8) + fleet_summary(4) + step(1) + player_oh(num_players)
        ...
        # Action features from local planet + global context
        self.action_head = nn.Sequential(
            nn.Linear(embed_dim * 2, embed_dim), nn.ReLU(),
            nn.Linear(embed_dim, embed_dim), nn.ReLU(),
        )
        
        self.send_head = nn.Linear(embed_dim, 1)  # logit for send/no-send
        self.ship_head = nn.Linear(embed_dim, 1)  # sigmoid for ship ratio

        # Pointer network: query from action_feat, keys from planet embeddings
    ...
    def encode_state(self, planets, fleets, player_id, step,
        ...
        Args:
            planets: [B, N, 7] normalized planet features
            fleets: [B, M, 7] normalized fleet features
            player_id: [B] player IDs
            step: [B] game step
            planet_mask: [B, N] bool mask for valid planets
            fleet_mask: [B, M] bool mask for valid fleets
            
        Returns:
            planet_emb: [B, N, D] planet embeddings
            global_feat: [B, D] global context
            value: [B] state value
        """
        B, N, _ = planets.shape
        ...
        if planet_mask is not None:
            ...
            planet_pool = planet_emb.sum(dim=1) / count
        else:
        ...
        planet_summary = torch.cat([
            my_m.sum(1, keepdim=True).float(),
            en_m.sum(1, keepdim=True).float(),
        ...
        if fleets.size(1) > 0:
            ...
            if fleet_mask is not None:
                fm = fm & fleet_mask
                fe = fe & fleet_mask
            fleet_summary = torch.cat([
                fm.sum(1, keepdim=True).float(),
                fe.sum(1, keepdim=True).float(),
                (fs * fm.float()).sum(1, keepdim=True),
                (fs * fe.float()).sum(1, keepdim=True),
            ], dim=1)
        else:
            fleet_summary = torch.zeros(B, 4, device=planets.device)

        pid_oh = F.one_hot(player_id.clamp(0, self.num_players - 1), self.num_players).float()
        step_norm = step.unsqueeze(1) / 500.0
        global_input = torch.cat([planet_summary, fleet_summary, step_norm, pid_oh], dim=1)
    ...
    def forward(self, planets, fleets, player_id, step,
        ...
        Returns dict with:
        - send_logits: [B, N, 1]
        - target_logits: [B, N, N] (pointer network over all planets)
        ...
        # Attention scores: [B, N_src, N_tgt]
        ...
        # Mask own planets as targets (don't attack yourself)
        if owned_mask is not None:
            own_expand = owned_mask.unsqueeze(1).expand(-1, N, -1)
    ...
    def get_action_and_value(self, planets, fleets, player_id, step,
        ...
        Returns:
            actions: list of action lists per batch
            log_probs: [B] total log probability
        ...
        # Send decision: Bernoulli distribution
        if owned_mask is not None:
            send_logits_masked = send_logits.masked_fill(~owned_mask, -1e9)
            send_probs = torch.sigmoid(send_logits_masked)
        else:
        ...
        # Target selection: Categorical distribution (pointer network)
        ...
        # Combine log probs and entropy (only for owned planets that send)
        if owned_mask is not None:
    ...
    def evaluate_actions(self, planets, fleets, player_id, step,
        ...
        Returns:
            total_lp: [B] total log probability
            total_ent: [B] total entropy
            value: [B]
        """
        out = self.forward(planets, fleets, player_id, step,
                          planet_mask, fleet_mask, owned_mask)
