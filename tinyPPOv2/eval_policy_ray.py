...
from tinyPPOv2.agent import AutoregressiveAgent
...
def _eval_part(config: dict[str, Any], games: int, seed: int) -> dict[str, float]:
    ...
    result = run_matchups(
        lambda: AutoregressiveAgent(
            ckpt,
            device=config["device"],
            deterministic=not config["stochastic"],
            continue_bias=config["continue_bias"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate tinyPPOv2 autoregressive policy against regular.")
    ...
    parser.add_argument("--continue-biases", default="0.0")
    ...
    for temperature in parse_float_list(args.temperatures):
        for continue_bias in parse_float_list(args.continue_biases):
            configs.append(
                {
    ...
    for row in all_rows:
        print(
            f"bias={row['continue_bias']} temp={row['temperature']} "
