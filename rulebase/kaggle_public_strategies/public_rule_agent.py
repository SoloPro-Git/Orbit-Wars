...
class PublicRuleAgent:
        ...
        for target_id, plan in plans.items():
            ...
            for source, _ in closest_planets_to_target(local.mine, target):
                if source.id == target.id or source.id in exhausted_planet_ids:
                    continue
                ships_to_send = max(MIN_SHIPS_MINE_ATTACK, ships_needed)
    ...
    def _append_attacks(
        ...
        for source in sorted(local.mine, key=lambda p: p.ships, reverse=True):
            if source.id in exhausted_planet_ids:
                continue
            if self._available_ships(source, under_attack) < MIN_SHIPS_MINE_ATTACK:
                continue

            candidate_targets = [
                target
                for target in local.targets
                if target.id not in local.comet_planet_ids
            ]
    ...
    def _base_ships_needed(self, target: Planet, local: LocalObs) -> int | None:
        ...
        if target.owner != -1:
            needed_now += int(3 * target.production)
        ...
        if enemy_competing > 0:
            ...
            if target.owner != -1:
                needed_with_race += int(3 * target.production)
            needed_now = max(needed_now, needed_with_race)

        if len(local.mine) < len(local.planets) * 0.75 and en_route >= needed_now:
            return None
        return max(MIN_SHIPS_MINE_ATTACK, needed_now - en_route)
    ...
    def _try_single_attack(
        ...
        if angle is None or arrive_tick is None:
            return False
        if sun_collision(source, total_ships, angle):
            return False
    ...
    def _try_coop_attack(
        ...
        base_ships = self._base_ships_needed(target, local)
        if base_ships is None or target.ships < MIN_SHIPS_TARGET_COOP_ATTACK or len(local.mine) <= 1:
            return False
        ...
        for planet, _ in closest_planets_to_target(local.mine, target):
            ...
            if available < MIN_SHIPS_MINE_ATTACK:
                continue
    ...
    def _build_coop_plan(
        ...
        for attack in attacking_planets:
            ...
            if angle is None or arrive_tick is None:
                continue
            if sun_collision(source, ships, angle):
                continue
